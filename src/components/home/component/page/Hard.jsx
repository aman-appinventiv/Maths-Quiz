...
function Hard() {
  ...
  const hardQuestion = location.state.hard.hard
  const name = location.state.name.name
  //   console.log(name)
  const [Index, setIndex] = useState(0)
  const [score, setScore] = useState(0)
  const [showResult, setShowResult] = useState(false)
  const currentQuestion = hardQuestion[Index]
  ...
  const handleAnswer = (selectedAnswer) => {
    ...
    if (nextIndex < hardQuestion.length) {
      setIndex(nextIndex)
    } else {
      setShowResult(true)
    }
    ...
    <div>
      ...
      {!showResult ? (
        <div className="border border-white p-10 rounded-md">
          <div>Total question:- {hardQuestion.length}</div>
        ...
        <div className="border border-white p-10 rounded-md">
          ...
          <p className="m-5 text-2xl font-bold p-2">
            Your Score:- {score}/{hardQuestion.length}
          </p>
